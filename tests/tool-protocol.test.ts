...
describe("function tool prompt protocol", (): void => {
  it("places tool definitions and untrusted input in a strict prompt envelope", (): void => {
    ...
    expect(prompt).toContain("<web2api_function_calls>");
  ...
  it("rejects protocol output that calls an undeclared function", (): void => {
    expect((): void => {
      parseFunctionResponse(
        '<web2api_function_calls>{"calls":[{"call_id":"call_bad","name":"delete_everything","arguments":{}}]}</web2api_function_calls>',
        [weatherTool]
      );
  ...
  it("rejects protocol tags surrounded by assistant commentary", (): void => {
    expect((): void => {
      parseFunctionResponse(
        'I will call it now. <web2api_function_calls>{"calls":[{"call_id":"call_weather","name":"get_weather","arguments":{"city":"Paris"}}]}</web2api_function_calls>',
        [weatherTool]
      );
  ...
  it("rejects multiple calls when parallel calls are disabled", (): void => {
    expect((): void => {
      parseFunctionResponse(
        '<web2api_function_calls>{"calls":[{"call_id":"call_paris","name":"get_weather","arguments":{"city":"Paris"}},{"call_id":"call_london","name":"get_weather","arguments":{"city":"London"}}]}</web2api_function_calls>',
        [weatherTool],
        "auto",
        false
      );
